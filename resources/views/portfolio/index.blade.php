...
@section('content')
    ...
    <section class="relative h-64 bg-black flex items-center justify-center">
        <div class="absolute inset-0">
            <img src="/placeholder.svg?height=400&width=1920" alt="Portfolio" class="w-full h-full object-cover opacity-50">
        </div>
        ...
        <div class="container-custom">
            ...
            <div class="max-w-md mx-auto mb-12">
                <form method="GET" class="relative">
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Search portfolio..."
            ...
            @if ($portfolios->count() > 0)
                @php
                    $chunks = $portfolios->chunk(ceil($portfolios->count() / 3));
                @endphp

                <div class="mt-16 grid gap-4 sm:grid-cols-2 md:grid-cols-3 lg:gap-6">
                    @foreach ($chunks as $chunk)
                        <div class="grid gap-4 lg:gap-6">
                            @foreach ($chunk as $portfolio)
                                <div class="relative overflow-hidden rounded-lg shadow-lg group"
                                    data-category="{{ $portfolio->category->slug }}">
                                    <img src="{{ $portfolio->thumbnail_path ? Storage::url($portfolio->thumbnail_path) : '/placeholder.svg' }}"
                                        alt="{{ $portfolio->title }}"
                                        class="w-full block object-cover transition-transform duration-300 group-hover:scale-110" />

                                    <div
                                        class="absolute inset-0 bg-black bg-opacity-60 opacity-0 group-hover:opacity-100 transition duration-300 flex items-center justify-center text-white text-center px-4">
                                        <div>
                                            <h3 class="text-xl font-semibold mb-2">{{ $portfolio->title }}</h3>
                                            <p class="text-sm mb-4">{{ $portfolio->category->name }}</p>
                                            <div class="flex gap-2 justify-center">
                                                <button
                                                    onclick="openLightbox('{{ $portfolio->image_path ? Storage::url($portfolio->image_path) : '/placeholder.svg?height=800&width=600' }}')"
